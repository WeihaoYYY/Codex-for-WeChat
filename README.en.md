...
Browser tasks originating from WeChat first try the user's everyday Chrome through computer-use so existing site logins can be reused. If Chrome is unavailable, access is denied, or it fails before any consequential action, Codex retries once with the in-app browser. The WeChat user is never asked to `@Browser`. Cookies are not copied between these browsers, and an uncertain submission, send, publish, purchase, deletion, or upload is never retried automatically in another browser. This local deployment disables the isolated bridge browser, so the `weixin_browser` tool schema is not loaded.
