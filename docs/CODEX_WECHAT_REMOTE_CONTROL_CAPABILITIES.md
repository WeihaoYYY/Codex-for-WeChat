...
- 微信网页任务只按“日常 Chrome → Codex 内置浏览器”的顺序尝试，不要求微信用户 `@Browser`。两者登录数据相互独立；只有在尚未产生外部后果时才允许切换浏览器重试，提交、发送、发布、购买、删除或上传结果不确定时必须停下确认，避免重复操作。本机配置关闭桥接隔离浏览器，不加载 `weixin_browser` 工具定义。
