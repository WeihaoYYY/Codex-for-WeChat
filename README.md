...
过程进度默认开启。Codex 处理长任务时会持续向微信发送中间进度，Web 端则折叠显示处理过程和用时，最终答案保持完整。只有一轮成功完成后，微信才会额外收到一条独立的 `本次任务结束`；等待审批、超时或失败不会误报成功结束。
...
日常使用不需要记命令。只要该 Project 位于“允许的工作目录”之下，直接发送：
...
Codex for WeChat 会从 Codex 任务列表识别 `身体恢复` 对应的目录，为该联系人自动创建或复用一条独立的微信受管会话，并执行消息里的任务。也可以发送 `切换到身体恢复`，之后直接继续聊天。项目名必须能唯一匹配目录名；匹配到多个目录时会停止并要求使用更完整的名称。

微信不会接管 Codex Desktop 正在写入的同一个 thread，而是在相同 Project 目录下维护自己的会话，从而避免 active-writer 冲突。
...
文件修改、命令执行、新域名、上传和提交类网页操作可能暂停等待审批。请先阅读微信里的操作摘要，再决定 `/approve A1` 或 `/deny A1`；不要盲目批准。等待微信审批期间会暂停 Codex 执行超时计时，批准或拒绝后再恢复计时。

受信任的单人部署可以在设置中启用“高信任微信本机操作”，并将 Exec 沙箱设为“完整访问”。此模式自动允许 Codex 执行本机命令和修改允许工作目录内的文件；在域名列表中填写 `*` 可允许访问所有公共网站。私网/本机地址仍被阻止，网页上传、提交、发布、付款和删除等外部后果操作仍需单次确认。

微信发起的网页任务会先通过 computer-use 尝试日常 Chrome，以复用已有的网站登录状态；如果 Chrome 不可用、访问被拒绝，或在产生任何外部后果前失败，则使用 Codex 内置浏览器重试一次。微信用户无需 `@Browser`。两种浏览器的 Cookie 不会自动互相复制；如果任务已执行到提交、发送、发布、购买、删除或上传且结果不确定，不会跨浏览器自动重试，以免重复操作。本机部署关闭桥接隔离浏览器，不会加载 `weixin_browser` 工具定义。
...
- `项目名：任务` 会从 Codex 保存的任务中发现允许目录内的 Project，并自动创建或复用该 Project 的微信受管会话。
- `切换到项目名` 会只切换 Project；之后的普通消息直接发往该 Project 的微信受管会话。

## 微信内命令

```text
项目名：任务                  一句话切换项目并执行任务，例如“身体恢复：总结今天的资料”
任务到项目名                  自然指定任务去向，例如“帮我总结视频内容到身体恢复”
切换到项目名                  使用中文切换到指定项目
/help                         查看命令
...
微信 turn 使用 `approvalPolicy: "on-request"`。Codex 请求执行本机命令、修改文件或使用需要确认的浏览器操作时，服务会向同一微信联系人发送 `A1` 这类一次性编号；只有该联系人可以用 `/approve A1`、`/approve-session A1` 或 `/deny A1` 回答。编号默认 10 分钟过期且只能使用一次；等待该决定的时间不计入 Codex 执行超时。`/stop` 会取消该联系人的所有待确认操作。浏览器提交和文件上传只接受单次 `/approve`，不能升级为会话级授权。

启用“高信任微信本机操作”后，命令和文件修改会自动接受，不再生成 `A1`；浏览器外部后果操作仍保持上述审批边界。此设置只适用于服务仅监听本机且微信联系人授权范围经过核对的个人设备。
